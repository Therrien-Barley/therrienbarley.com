...
var Element = require(__dirname + '/models').Element;
...
function download(req, res, _id){
	...
	Element
		.find({ type: tumblr, collection_id: _id })
		.sort('-timestamp')
...
function fetchCallback(host, tag, limit, offset, req, res, fetched_posts, inserted_posts, total_posts, col){
	fetched_total_posts++;
	...
	if(fetched_total_posts >= total_posts){
		...
		if(inserted_posts >= fetched_posts){
			fetch(host, tag, limit, fetched_total_posts, req, res, col);
		}
	}
}
