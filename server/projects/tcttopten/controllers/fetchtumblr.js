...
function gettoptentumblrposts(req, res){
...
function fetchTumblr(limit, offset, req, res){
	...
	console.log('calling fetchTumblr for tcttopten');

	tumblr.get('/posts', {hostname: 'troyconradtherrien.tumblr.com', offset: offset, limit: limit, tag: 'topten'}, function(json){
		...
		if(json.posts.length == 0){
			...
			for(var i = 0; i < fetched_posts; i++){
				...
				for(var t = 0; t < json.posts[i].tags.length; t++){
	                ...
	                if(json.posts[i].tags[t] == 'topten' || json.posts[i].tags[t] == 't'){
