...
var POSTS_COLLECTION = 'tumblrposts';

db.open(function(err, db) {
	console.log('opening DB tcttopten');
    if(!err) {
        console.log("Connected to tcttopten database");
        db.collection(POSTS_COLLECTION, {safe:true}, function(err, collection) {
            if (err) {
                console.log("Error trying to open toptentumblrposts collection with error: " + err);
            }
        });
    }
});
...
var tumblr = new Tumblr(
  {
    consumerKey: 'QqEwqwYW4ZbkNtlrL050ETFdkN4YO0gEowuyBcCSFWrzVtt1hk'
  }, "troyconradtherrien.tumblr.com"
...
exports.fetchtoptentumblrposts = function(req, res){
    var col = req.params.collection;
    return gettoptentumblrposts(col);
}



function gettoptentumblrposts(col){

    console.log('gettoptentumblrposts()');

	post_offset = iter*post_limit;


    tumblr.get('/posts', {hostname: 'troyconradtherrien.tumblr.com', offset: post_offset, limit: post_limit}, function(json){
...
function returnDistribution(returnObject, req, res, total_posts, taxonomy){
    ...
    switch(taxonomy){
        case 'categories':
            res.render('../projects/tcttopten/views/categories/distribution', {
...
function returnElements(returnObject, req, res, number, taxonomy){
    ...
    switch(taxonomy){
        case 'categories':
            res.render('../projects/tcttopten/views/categories/elements', {
...
exports.getElementsByTags = function(req, res, taxonomy){
    getElements(req, res, returnElements, POSTS_COLLECTION, taxonomy);
}
...
exports.getElement = function(req, res, id){
    ...
    db.collection(POSTS_COLLECTION, function(err, collection) {
...
exports.renderTermsDistribution = function(req, res){
    ...
    db.collection(col, function(err, collection) {
        collection.find().toArray(function(err, items){
            ...
            res.render('../projects/tcttopten/views/terms/distribution', {
                ...
                db.collection(POSTS_COLLECTION, function(err, collection) {
                    collection.update({ id: rtnObjID }, { $set: { 'fragments': returnObject[tag][i].fragments } }, { safe:true }, function(err) {
                        if(err){
                            ...
                            console.log('successfully added new quotes to toptentumblrposts collection with id: '+ rtnObjID);
                        }
                    });
                });
            }     
                ...
                db.collection(POSTS_COLLECTION, function(err, collection) {
                    collection.update({ id: rtnObjID }, { $set: { 'fragments': returnObject[tag][i].fragments } }, { safe:true }, function(err) {
                        if(err){
                            ...
                            console.log('successfully added new images to toptentumblrposts collection with id: '+ rtnObjID);
                        }
                    });
                });
            } 
...
exports.getFragments = function(req, res, taxonomy, fragment, tag){
    ...
    switch(fragment){
        case 'quotes':
            ...
            getElements(req, res, renderFragmentQuotes, POSTS_COLLECTION, taxonomy, tag);
            break;
        case 'images':
            console.log('images');
            getElements(req, res, renderFragmentImages, POSTS_COLLECTION, taxonomy, tag);
            break;
